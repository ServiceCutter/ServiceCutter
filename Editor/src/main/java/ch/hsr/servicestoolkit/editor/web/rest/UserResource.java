...
import ch.hsr.servicestoolkit.editor.repository.AuthorityRepository;
...
import ch.hsr.servicestoolkit.editor.web.rest.dto.ManagedUserDTO;
import ch.hsr.servicestoolkit.editor.web.rest.util.HeaderUtil;
import ch.hsr.servicestoolkit.editor.web.rest.util.PaginationUtil;
...
public class UserResource {
	...
	public ResponseEntity<User> createUser(@RequestBody final User user) throws URISyntaxException {
		...
		return ResponseEntity.created(new URI("/api/users/" + result.getId())).headers(HeaderUtil.createEntityCreationAlert("user", result.getId().toString())).body(result);
	}
	...
	@Transactional
	@Secured(AuthoritiesConstants.ADMIN)
	public ResponseEntity<ManagedUserDTO> updateUser(@RequestBody final ManagedUserDTO managedUserDTO) throws URISyntaxException {
	...
	public ResponseEntity<List<ManagedUserDTO>> getAllUsers(final Pageable pageable) throws URISyntaxException {
	...
	@Timed
	public ResponseEntity<ManagedUserDTO> getUser(@PathVariable final String login) {
		log.debug("REST request to get User : {}", login);
		return userService.getUserWithAuthoritiesByLogin(login).map(user -> new ManagedUserDTO(user)).map(userDTO -> new ResponseEntity<>(userDTO, HttpStatus.OK))
